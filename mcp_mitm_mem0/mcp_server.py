...
@mcp.tool()
async def search_memories(query: str, user_id: str | None = None, limit: int = 10) -> list[dict[str, Any]]:
    """
...
async def get_user_memories(user_id: str) -> Resource:
    ...
    try:
        ...
        for i, memory in enumerate(memories, 1):
            ...
            content += f"- Content: {memory.get('memory', memory.get('content', 'N/A'))}\n"
...
@mcp.tool()
async def analyze_conversations(user_id: str | None = None, limit: int = 20) -> dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info("Conversation analysis completed", insights=len(results.get("insights", [])))
        return results
    except Exception as e:
